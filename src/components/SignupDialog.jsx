...
function SignupDialog({open,setOpen}) {
        ...
        if(formValidation("name",newEntry.name)&&formValidation('email',newEntry.email)&&formValidation('phone',newEntry.phone)&&formValidation('password',newEntry.password)){
            ...
            try{
                ...
                if(response){
                    ...
                    setFeedBack('Email Id allready in use!!!. Try Logging in ')
                    setLoading(false)
                }
            }
            catch(err){
  ...
  return (
    <>
    <Dialog fullWidth maxWidth={'sm'} style={{padding:'10px'}} open={open} onClose={()=>{setOpen(false);setFeedBack('');setNewEntry({name:'',email:'',password:'',orders:[],cartItems:[],phone:''})}}>
